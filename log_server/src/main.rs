...
async fn echo(req: Request<Body>) -> Result<Response<Body>, hyper::Error> {
  ...
  match (req.method(), req.uri().path()) {
    ...
    (&Method::POST, "/upload") => {
      ...
      let v: Vec<_> = Connection::simple_db_query(&client, "upload_hash", &conn.filehash, CONFIG.get("database").unwrap(), CONFIG.get("registry").unwrap()).await.collect().await;
      if v.len() > 0 {
    ...
    (&Method::POST, "/check") => {
      ...
      let v: Vec<_> = Connection::simple_db_query(&client, "id", &conn.filehash, CONFIG.get("database").unwrap(), CONFIG.get("registry").unwrap()).await.collect().await;
      if v.len() > 0 {
    ...
    (&Method::POST, "/cleanup") => {
      ...
      for f in files {
        ...
        let v: Vec<_> = Connection::simple_db_query(&client, "upload_path", filepath.to_str().unwrap(), CONFIG.get("database").unwrap(), CONFIG.get("registry").unwrap()).await.collect().await;
        if v.len() == 0 {
